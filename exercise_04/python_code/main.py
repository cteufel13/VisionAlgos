...
def main(
    ...
    if np.abs(rotation_img2_deg) > 1e-6:
        ...
        height = img2.shape[0]
        width = img2.shape[1]
        R = cv2.getRotationMatrix2D((height // 2, width // 2), rotation_img2_deg, 1.0)
        corners = np.array(
            [
                [0, 0, 1],  # top-left
                [width, 0, 1],  # top-right
        ...
        new_positions = R @ corners
        xs = new_positions[0]
        ys = new_positions[1]

        new_width = int(np.ceil(xs.max() - xs.min()))
        new_height = int(np.ceil(ys.max() - ys.min()))

        dh = int(np.ceil((new_height - height) / 2))
        dw = int(np.ceil((new_width - width) / 2))
        image_padded = cv2.copyMakeBorder(img2, dh, dh, dw, dw, cv2.BORDER_CONSTANT, 0)
        rotate_img = cv2.getRotationMatrix2D(
            (new_width / 2, new_height / 2), rotation_img2_deg, 1.0
        )
        img2 = cv2.warpAffine(
            src=image_padded, M=rotate_img, dsize=(new_width, new_height)
        )
